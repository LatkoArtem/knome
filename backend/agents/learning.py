...
_WANT_LEARN_KW = [
    ...
    "давно не вчився", "давно не вчилась", "давно не читав", "давно не читала",
    "давно не займався", "давно не займалась", "треба б вчитися",
    "треба взятись за навчання", "треба взятись за книг", "пора вчитися",
    "взятись за себе", "взятися за себе",
]
...
async def process(user_message: str, user_id: str, context: dict | None = None) -> tuple[str, list]:
    ...
    if any(kw in text for kw in _WANT_LEARN_KW):
        ...
        return (
            "Жодних тем для повторення ще немає — час починати навчання! 📚\n"
            "Почни перше заняття щоб вчитись ефективно:\n"
            "«позаймався Python 30 хвилин» — і я буду відстежувати прогрес!"
        ), []
